...
fn main() {
    ...
    let cluster_nodes_config = config.get("cluster")
        .cloned()
        .unwrap_or_else(|| "".to_string());
    let cluster_nodes: Vec<String> = cluster_nodes_config.split(',').map(|s| s.trim().to_string()).collect();

    for node in cluster_nodes {
        let current = if node == format!("{}:{}", host, port_num) { "* " } else { "  " };
